            ...
            if (line[0:4] == "ATOM"):
                ...
                if len(g) >= 2:
                    try:
                        ...
                        pass
            elif line[0:3] == "ENDMDL":
                self.add_model()
            elif line[0:3] == "TER":
                self.add_chain()
        self.add_model()
        self.molecule = Molecule(os.path.splitext(file_name)[0], self.models)
...
mol = PdbParser(sys.argv[1]).molecule
num_atom = 1
num_residue = 1
f = open(sys.argv[2], 'w+')
for chain in mol[0]:
    for residue in chain:
        for atom in residue:
            f.write("ATOM%7i  %-4s%3s%2s%4i%12.3lf%8.3lf%8.3lf%6.2f%6.2f%12c  \n" % \
                (num_atom, atom.name , residue.name , 'A', num_residue, atom[0], atom[1], atom[2] , 1.00 , 0.00, atom.name[0]))
            num_atom += 1
        num_residue += 1
f.close()
